...
st.title("Economic Event Impact Simulator (2020–2035)")
...
for year in years:
    with st.expander(f"Configure Year {year}", expanded=False):
        ...
        duration = st.slider(f"Duration of the event (months)", 1, 24, 3, key=f"duration_{year}")
...
if st.button("Simulate Impact"):
    if not events_data or not selected_indicators:
        ...
        predictions_df = model_predict(events_df, selected_indicators)

        st.subheader("Predicted Values")
        st.dataframe(predictions_df)

        st.subheader("Plots by Indicator")
        for indicator in selected_indicators:
            subset = predictions_df[predictions_df["Indicator"] == indicator]
            st.line_chart(subset.set_index("Year")["Predicted Value"])
